...
export function TaskStatusButton({ clinicId, task }: { clinicId: number; task: AssignedTask }) {
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const utils = trpc.useUtils();
  const mutation = trpc.tasks.updateStatus.useMutation({
    onSuccess: async () => {
      setErrorMessage(null);
      await utils.tasks.assigned.invalidate({ clinicId });
    },
    onError: (error) => {
  ...
  return (
    <span className="staff-task-action-wrap">
      <button
        ...
        onClick={() => {
          setErrorMessage(null);
          mutation.mutate({ clinicId, taskId: task.id, action });
        }}
