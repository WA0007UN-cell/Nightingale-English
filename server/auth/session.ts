...
export async function getActorFromSession(request: Request): Promise<SessionActor | undefined> {
  ...
  if (process.env.NODE_ENV !== "production" && previewToken) {
    try {
      ...
      if ((payload.preview === "synthetic_staff" || payload.preview === "synthetic_clinician" || payload.preview === "synthetic_patient") && Number.isInteger(userId) && userId > 0 && Number.isInteger(clinicId) && clinicId > 0) {
