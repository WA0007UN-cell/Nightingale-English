...
function createReader(membershipRole: "Clinician" | "Staff" | "Patient" | "Admin", patientUserId = 3): WorkspaceReader {
  return {
    ...
    async listEntries(clinicId, patientId) { return [
      ...
      { id: 103, clinicId, patientId, authorRole: "Staff", entryType: "escalation", visibility: "clinic", reviewState: "review_required", content: "Internal Staff escalation linked to source 101.", occurredAt: at },
    ]; },
...
describe("authorised workspace read", () => {
  it.each(["Clinician", "Staff"] as const)("returns same-clinic workspace records to %s", async (role) => {
    ...
    expect(result.entries).toHaveLength(3); expect(result.tasks).toHaveLength(1);
  });
  ...
  it("returns only patient-visible entries and no internal tasks to a linked patient", async () => {
    ...
    expect(result.entries.map((entry) => entry.id)).toEqual([102]);
    expect(result.entries.some((entry) => entry.entryType === "escalation" || entry.id === 101)).toBe(false);
